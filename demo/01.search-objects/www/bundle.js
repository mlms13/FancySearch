...
Main.main = function() {
	...
	var opts_suggestions = people;
	var opts_filter = function(person,search) {
		return Main.personToString(person).toLowerCase().indexOf(search.toLowerCase()) >= 0;
	};
	var opts_sugEq = function(a,b) {
		return thx__$Ord_Ord_$Impl_$.equal(thx_Strings.order,a.github,b.github);
	};
	var value = opts_limit;
	var value1 = opts_initValue;
	var config = fancy_search_defaults_AutocompleteDefaults.create(fancy_search_defaults_AutocompleteDefaults.filterSync(opts_suggestions,opts_filter,null == value ? haxe_ds_Option.None : haxe_ds_Option.Some(value)),opts_sugEq,thx_Functions.identity,null == value1 ? haxe_ds_Option.None : haxe_ds_Option.Some(value1),opts_initFilter,opts_minLength,opts_alwaysHighlight);
	var container = dots_Query.find(".fancy-container");
	var input = dots_Query.find(".fancy-container input");
	var search1 = new fancy_Search(config);
	var renderer = fancy_search_renderer_DomStringFilter.fromInput(input,search1,{ classes : fancy_search_defaults_ClassNameDefaults.defaults, keys : fancy_search_defaults_KeyboardDefaults.defaults, renderSuggestion : function(p,filter) {
		var doc = null;
		if(null == doc) {
			doc = window.document;
		}
		...
		return el;
	}, elements : { clearButton : haxe_ds_Option.None, failedCondition : haxe_ds_Option.None, loading : haxe_ds_Option.None, failed : haxe_ds_Option.None, noResults : haxe_ds_Option.Some(function() {
		var doc3 = null;
		if(null == doc3) {
		...
		if(null != children4) {
			var _g4 = 0;
			while(_g4 < children4.length) {
				var child3 = children4[_g4];
				++_g4;
	...
	search1.values.next(function(val) {
		var tmp;
		switch(val[1]) {
		case 0:
			var str = val[2];
		...
		case 1:
			var person1 = val[2];
			tmp = Main.personToString(person1);
			break;
		}
		input.value = tmp;
	}).run();
...
var fancy_search_renderer_DomStringFilter = function() { };
$hxClasses["fancy.search.renderer.DomStringFilter"] = fancy_search_renderer_DomStringFilter;
fancy_search_renderer_DomStringFilter.__name__ = ["fancy","search","renderer","DomStringFilter"];
fancy_search_renderer_DomStringFilter.renderStringSuggestion = function(sugg,filter) {
	var doc = null;
	if(null == doc) {
		doc = window.document;
	}
	...
	var textContent = sugg;
	if(null != textContent) {
		el.appendChild(doc.createTextNode(textContent));
	}
	return el;
};
fancy_search_renderer_DomStringFilter.renderMenuItem = function(config,renderCfg,filter,dispatch,highlighted,sugg) {
	...
	var children = [renderCfg.renderSuggestion(sugg,filter)];
	if(null != children) {
		var _g21 = 0;
...
fancy_search_renderer_DomStringFilter.renderMenu = function(cfg,dispatch,state) {
	var _g = state.menu;
	switch(_g[1]) {
	case 0:
		switch(_g[2][1]) {
		...
		case 1:
			...
			var children1 = thx_Options.toArray(thx_Options.map(cfg.elements.failedCondition,function(lazy) {
				return lazy(reason);
			}));
			if(null != children1) {
				var _g23 = 0;
	...
	case 1:
		...
		case 0:
			...
			var children2 = thx_Options.toArray(thx_Options.map(cfg.elements.loading,function(lazy1) {
				return lazy1();
			}));
			if(null != children2) {
				var _g25 = 0;
		...
		case 1:
			...
			var children3 = thx_Options.toArray(thx_Options.map(cfg.elements.noResults,function(lazy2) {
				return lazy2();
			}));
			if(null != children3) {
				var _g27 = 0;
		...
		case 2:
			...
			var children4 = thx_Options.toArray(thx_Options.map(cfg.elements.failed,function(lazy3) {
				return lazy3();
			}));
			if(null != children4) {
				var _g29 = 0;
		...
		case 3:
			...
			var children5 = function(sugg) {
				return fancy_search_renderer_DomStringFilter.renderMenuItem(config,renderCfg,filter,dispatch1,highlighted1,sugg);
			};
			var children6 = thx__$Nel_Nel_$Impl_$.toArray(suggs).map(children5);
			if(null != children6) {
				var _g51 = 0;
				while(_g51 < children6.length) {
					var child5 = children6[_g51];
					++_g51;
			...
			if(null != children7) {
				var _g42 = 0;
				while(_g42 < children7.length) {
					var child6 = children7[_g42];
					++_g42;
...
fancy_search_renderer_DomStringFilter.fromInput = function(input,search,cfg) {
	var cfg1 = cfg;
	var dispatch = function(act) {
		search.store.dispatch(act,{ fileName : "DomStringFilter.hx", lineNumber : 61, className : "fancy.search.renderer.DomStringFilter", methodName : "fromInput"});
	};
	var menu = function(state) {
		return fancy_search_renderer_DomStringFilter.renderMenu(cfg1,dispatch,state);
	};
	var menu1 = search.store.stream().map(menu);
	input.addEventListener("focus",function(_) {
		search.store.dispatch(fancy_search_Action.SetFilter(input.value),{ fileName : "DomStringFilter.hx", lineNumber : 64, className : "fancy.search.renderer.DomStringFilter", methodName : "fromInput"});
		search.store.dispatch(fancy_search_Action.OpenMenu,{ fileName : "DomStringFilter.hx", lineNumber : 65, className : "fancy.search.renderer.DomStringFilter", methodName : "fromInput"});
	});
	input.addEventListener("blur",function(_1) {
		search.store.dispatch(fancy_search_Action.CloseMenu,{ fileName : "DomStringFilter.hx", lineNumber : 67, className : "fancy.search.renderer.DomStringFilter", methodName : "fromInput"});
	});
	input.addEventListener("input",function(_2) {
		search.store.dispatch(fancy_search_Action.SetFilter(input.value),{ fileName : "DomStringFilter.hx", lineNumber : 68, className : "fancy.search.renderer.DomStringFilter", methodName : "fromInput"});
	});
	input.addEventListener("keydown",function(e) {
		...
		if(cfg.keys.highlightUp(keyWithModifiers)) {
			e.stopPropagation();
			e.preventDefault();
			search.store.dispatch(fancy_search_Action.ChangeHighlight(fancy_search_HighlightChangeType.Move(fancy_search_Direction.Up)),{ fileName : "DomStringFilter.hx", lineNumber : 74, className : "fancy.search.renderer.DomStringFilter", methodName : "fromInput"});
		} else if(cfg.keys.highlightDown(keyWithModifiers)) {
			e.stopPropagation();
			e.preventDefault();
			search.store.dispatch(fancy_search_Action.ChangeHighlight(fancy_search_HighlightChangeType.Move(fancy_search_Direction.Down)),{ fileName : "DomStringFilter.hx", lineNumber : 78, className : "fancy.search.renderer.DomStringFilter", methodName : "fromInput"});
		} else if(cfg.keys.choose(keyWithModifiers)) {
			e.stopPropagation();
			e.preventDefault();
			search.store.dispatch(fancy_search_Action.ChooseCurrent,{ fileName : "DomStringFilter.hx", lineNumber : 82, className : "fancy.search.renderer.DomStringFilter", methodName : "fromInput"});
		} else if(cfg.keys.close(keyWithModifiers)) {
			e.stopPropagation();
			e.preventDefault();
			search.store.dispatch(fancy_search_Action.CloseMenu,{ fileName : "DomStringFilter.hx", lineNumber : 86, className : "fancy.search.renderer.DomStringFilter", methodName : "fromInput"});
		}
	});
	return menu1;
};
